<artifact identifier="index-js-cloud-functions" type="application/vnd.ant.code" language="javascript" title="index.js - Cloud Functions Al Bourakh"> const functions = require('firebase-functions'); const admin = require('firebase-admin'); 
admin.initializeApp(); const db = admin.firestore();
// ========================================== // CONFIGURATION SYSTÈME // ==========================================
async function getSystemParams() { try { const doc = await db.collection('parametres').doc('config').get(); if (doc.exists) { return doc.data(); } return { assignationAutomatique: true, delaiReassignation: 10, rayonRecherche: 10, notificationsActives: true }; } catch (error) { console.error('Erreur récupération paramètres:', error); return { assignationAutomatique: true, delaiReassignation: 10, rayonRecherche: 10, notificationsActives: true }; } }
// ========================================== // 1. ASSIGNATION AUTOMATIQUE // ==========================================
exports.assignerChauffeurAutomatique = functions.firestore .document('reservations/{reservationId}') .onCreate(async (snap, context) => { const reservation = snap.data(); const reservationId = context.params.reservationId;
...
exports.assignerChauffeurManuel = functions.https.onCall(async (data, context) => { if (!context.auth) { throw new functions.https.HttpsError('unauthenticated', 'Utilisateur non authentifié'); }
const { reservationId, chauffeurId } = data;
if (!reservationId || !chauffeurId) { throw new functions.https.HttpsError('invalid-argument', 'reservationId et chauffeurId requis'); }
try { const reservationDoc = await db.collection('reservations').doc(reservationId).get(); if (!reservationDoc.exists) { throw new functions.https.HttpsError('not-found', 'Réservation non trouvée'); }
...
} catch (error) { console.error('❌ Erreur assignation manuelle:', error); throw new functions.https.HttpsError('internal', error.message); } });
// ========================================== // 3. SYSTÈME DE FALLBACK (Réassignation) // ==========================================
exports.verifierAssignationTimeout = functions.pubsub .schedule('every 5 minutes') .onRun(async (context) => { console.log('🔍 Vérification des réservations non acceptées...');
const params = await getSystemParams();
...
async function reassignerChauffeur(reservationId, reservation) { try { if (reservation.chauffeurAssigne) { await db.collection('drivers').doc(reservation.chauffeurAssigne).update({ statut: 'disponible', reservationEnCours: null });
  await db.collection('notifications').add({
...
} catch (error) { console.error(❌ Erreur réassignation ${reservationId}:, error); } }
// ========================================== // 4. TERMINER UNE COURSE // ==========================================
exports.terminerCourse = functions.https.onCall(async (data, context) => { if (!context.auth) { throw new functions.https.HttpsError('unauthenticated', 'Non authentifié'); }
const { reservationId, chauffeurId } = data;
try { await db.collection('reservations').doc(reservationId).update({ statut: 'terminee', dateTerminaison: admin.firestore.FieldValue.serverTimestamp() });
await db.collection('drivers').doc(chauffeurId).update({
  statut: 'disponible',
  reservationEnCours: null,
...
exports.annulerReservation = functions.https.onCall(async (data, context) => { if (!context.auth) { throw new functions.https.HttpsError('unauthenticated', 'Non authentifié'); }
const { reservationId, raison } = data;
try { const reservationDoc = await db.collection('reservations').doc(reservationId).get(); const reservation = reservationDoc.data();
if (reservation.chauffeurAssigne) {
  await db.collection('drivers').doc(reservation.chauffeurAssigne).update({
    statut: 'disponible',
    reservationEnCours: null
...
function calculerDistance(lat1, lng1, lat2, lng2) { const R = 6371; const dLat = toRad(lat2 - lat1); const dLng = toRad(lng2 - lng1);
const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) + Math.cos(toRad(lat1)) * Math.cos(toRad(lat2)) * Math.sin(dLng / 2) * Math.sin(dLng / 2);
const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a)); const distance = R * c;
return distance; }
function toRad(valeur) { return valeur * Math.PI / 180; }
function getDefaultCoordsForAddress(address) { const coords = { 'plateau': { lat: 14.6928, lng: -17.4467 }, 'almadies': { lat: 14.7247, lng: -17.5050 }, 'sacre-coeur': { lat: 14.6937, lng: -17.4441 }, 'mermoz': { lat: 14.7108, lng: -17.4682 }, 'hlm': { lat: 14.7306, lng: -17.4542 }, 'yoff': { lat: 14.7500, lng: -17.4833 }, 'ouakam': { lat: 14.7200, lng: -17.4900 }, 'liberte': { lat: 14.7186, lng: -17.4697 }, 'medina': { lat: 14.6738, lng: -17.4387 }, 'parcelles': { lat: 14.7369, lng: -17.4731 }, 'grand-dakar': { lat: 14.6928, lng: -17.4580 }, 'fann': { lat: 14.6872, lng: -17.4535 }, 'point-e': { lat: 14.6953, lng: -17.4614 }, 'amitie': { lat: 14.7014, lng: -17.4647 }, 'ngor': { lat: 14.7517, lng: -17.5192 }, 'virage': { lat: 14.7314, lng: -17.4636 }, 'sicap': { lat: 14.7289, lng: -17.4594 } };
const addressLower = address.toLowerCase(); for (const [quartier, coordonnees] of Object.entries(coords)) { if (addressLower.includes(quartier)) { return coordonnees; } }
return { lat: 14.6928, lng: -17.4467 }; } </artifact>
